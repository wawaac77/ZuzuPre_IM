...
- (void)setupData {
    ...
    NSLog(@"height of cell is %f", height);
    
    for (NSInteger i = 0; i < _count; ++i) {
        //获取item的宽度
        // 这个位置itemWidthWithIndex 好像有点问题
         ...
         */
        CGFloat overItemWidth = width*2;
        NSLog(@"overItemWidth = %f", overItemWidth);
        if (i < _count - 2) {
            overItemWidth = width + [self itemWidthWithIndex:i+1];
            NSLog(@"overItemWidth i < _count-2 = %f", overItemWidth);
        }
        //isOverScreenWidth = maxX > ScreenWidth + overItemWidth;
        isOverScreenWidth = maxX > ScreenWidth;
        if (!isOverScreenWidth) {
            ...
            if (item) {
                ...
                NSLog(@"visibleItems%@", _visibleItems);
            }
        }
...
- (CGFloat)itemWidthWithIndex:(NSInteger)index {
    ...
    NSLog(@"itemWidthWithIdex width %f", width);
    return width;
}
...
- (MinScrollMenuItem *)editVisibleItemPlaceWithIndex:(NSInteger)index originX:(CGFloat)originX {
    ...
    NSLog(@"itemWidthWithIdex item %ld", item);
    return item;
}
